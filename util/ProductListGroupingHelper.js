jQuery.sap.declare("nw.epm.refapps.ext.shop.util.ProductListGroupingHelper");

sap.ui.base.Object.extend("nw.epm.refapps.ext.shop.util.ProductListGroupingHelper", {
    ...
    onGroupingDialogConfirmed : function(oEvent) {
        var mParams = oEvent.getParameters(), sSortPath, sFunctionName;
        if (mParams.groupItem) {
            sSortPath = mParams.groupItem.getKey();
            sFunctionName = sSortPath[0].toLowerCase() + sSortPath.slice(1);
            this._oTableOperations.setGrouping(new sap.ui.model.Sorter(sSortPath, mParams.groupDescending, jQuery
                    .proxy(this._oGroupFunctions[sFunctionName], this)));
        } else {
    ...
    _getNameGrouper : function(sName, oListItemContext) {
        ...
        var sText = this._oResourceBundle.getText("xfld.groupingLabel",
                [ sLabel, (oListItemContext.getProperty(sName)) ]);
        return {
    ...
    _oGroupFunctions : {
        ...
        averageRating : function(oListItemContext) {
            var iRating = parseInt(oListItemContext.getProperty("AverageRating"), 10), sText = "";
            if (iRating === 0) {
